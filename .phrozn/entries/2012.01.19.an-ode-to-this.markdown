...
Well, not really an &ldquo;ode&rdquo;. But the `this` keyword in Javascript has been
...
`prototype` property) at runtime (like Ruby). This is means that, if you think
...
For more info I really recommend vjeux's article [Javascript &endash; How Prototypal
Inheritance really
works](http://blog.vjeux.com/2011/javascript/how-prototypal-inheritance-really-works.html).

*\*Now this sounds really cool and nice and all, but it's actually not that
memory efficient. You have to take into account speed, retained size, etc.*
